...
(deref
 (re-frame/subscribe
  [:pull '[:dimension/name :dimension/formula
           {:unit/_dimension [:unit/name :unit/si-conversion-factor :unit/dimension :unit/si-base?]}]
   (ffirst (filter (comp (partial = {"m" 1 "s" -1}) second)
                   @(re-frame/subscribe
                     [:q '[:find ?id ?f
                           :where
