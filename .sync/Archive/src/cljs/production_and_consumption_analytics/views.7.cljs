          ...
          + ((juxt (partial transduce
                   ...
                   (partial transduce
                            (comp (filter (comp (partial = :consumption)
                                  ...
                                  (map (comp - last)))
                            +))
             (sequence (comp (filter (comp (partial = formula) second))
                             (map (fn [[t _ a sicf]]
                                   [t (* a sicf)])))
                       @ts)))
         interval (t/period
                             ...
                             [h-box
                              :children [(if (:total @edit)
                                             [input-text
                                              :model
                                              (str (+ total (case type
                                                             :production (* (:unit/si-conversion-factor selected-unit)
                                                                            (js/Number amount))
                                                             :consumption (- (* (:unit/si-conversion-factor selected-unit)
                                                                                (js/Number amount))))))
                                              :on-change
                                              (fn [nt]
                                               (swap! edit assoc :total false)
                                               (when-let [nt (js/Number nt)]
                                                (re-frame/dispatch
                                                 [:transact (let [[amount ntype]
                                                                  ((juxt abs #(cond (neg? %)
                                                                                    :consumption
                                                                                    (zero? %)
                                                                                    nil
                                                             ...
                                                             [{:db/id id
                                                               ...
                                                               :transactioner/amount (str (/ amount (:unit/si-conversion-factor selected-unit)))
                                                               :transactioner/timestamp (t/now)}])])))]
                                             [re-com.text/label
                                              :label (+ total (case type
                                                               :production (* (:unit/si-conversion-factor selected-unit)
                                                                              (js/Number amount))
                                                               :consumption (- (* (:unit/si-conversion-factor selected-unit)
                                                                                  (js/Number amount)))))
      ...
      :children [[h-box
                 ...
                 [h-box
                  :children [(let [timestamp-fn (if timestamp
                                                 ...
                                                 #(t/now))
                                   disable-submit?
                                   (or ((some-fn js/isNaN zero? neg?) (js/Number amount))
                                       (and timer-enabled
                                            (or timer-running
                                                (not timestamp))))]
                              [button
  ...
  (fn []
   [:div
    ...
    [v-box
     :children [[analysis]
                [v-box
                 :children (into [[md-circle-icon-button
