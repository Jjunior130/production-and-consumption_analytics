...
(defn select-dimension [& {:keys [label id]}]
 (let [dimension (re-frame/subscribe
       ...
       selected-units (re-frame/subscribe
                       [:q '[:find [?selected-unit ...]
                             :where
                             [?unit :unit/dimension ~id]
                             [?transactioner :transactioner/selected-unit ?selected-unit]]])]
  (fn [& {:keys [label id]}]
   (let [transactioner-count (->> @selected-units
                                  (sequence (comp (map (comp :db/id :unit/dimension))
                                                  (filter #(= id %))))
                                  count)
     ...
     :children [[analysis]
                [v-box
                 :children (concat [[md-circle-icon-button
                                     ...
                                     :on-click (let [d-ids @dimension-ids]
                                                (fn []
                                                    (reagent-modals/modal!
                                                     (for [dimension-id d-ids]
                                                          [select-dimension
                                                           :id dimension-id]))))
                                     :size :larger
                                     :emphasise? true
